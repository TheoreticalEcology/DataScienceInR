...
To perform this exercise use the **help** of the functions. Also have a look at the **examples** at the bottom of the help page of the function. You can obtain help for a specific function in several ways:
...
Load the package "EcoData" and have a look at the data using the `View()` function. This will open a new tab next to your R script. To return to your script, close the new tab or click on your script.

```{r, eval=TRUE}
library(EcoData)
# View(arthritis)

# Get  and rename the data
dat <- arthritis
```

Before, some data manipulation:

1) Coerce the variable `Improved` to an ordered factor with the levels: "None", "Some", "Marked" (in this order).

2) Coerce the variable `Treatment` to a factor. Because "Placebo"  comes alphabetically first than "Treated" we don't need to correct the order of the factors.
```{r, eval=TRUE}
# 1)
dat$Improved <- factor(dat$Improved, 
                       levels = c("None","Some","Marked"), # not alphabetically
                       ordered = TRUE)
# 2)
...
::: column-margin
**Hints for plotting** Change the color (look at the help to see how): You can see available colors with the function `colors()`. Change the bar widths (breaks). Play around to see how they change. When changing the bar widths, what do you notice on the y-axis? You can change the y-axis limits using `ylim`. Change the title and the x-axis name of your histogram using the arguments `main` and `xlab`.

To add lines, try the `abline()` function. Remember that `abline()` is a low level plotting function, which means it only adds an element to an existing plot! Instead of calculating the depicted values with a function and then pasting the values into `abline()`, you can also directly use the function in `abline()`. See an *example* below:
...
The notch is the confidence interval of the median, so 2\*the standard error of the median. We can roughly say that there is a significant difference when the confidence intervals do not overlap!
