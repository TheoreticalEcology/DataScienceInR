...
-   `tibble` objects are just an extension of data.frames. In the course we will use mostly data.frames, so it is better to transform the tibbles back do data.frames:
...
Idea: arranging the data following their order and using their rank 1..n for x und y, respectively and there two different rank correlation factors:
