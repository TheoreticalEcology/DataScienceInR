...
-   `complete.cases()`
-   `cor.test()`
-   `%>%` and `group_by()` and `summarize()` from the \`dplyr\` package (from tidyverse, check demonstration Part 2)
...
# "Where/ever/you/put/it/treedata.csv"
```
...
treedata <- treedata[treedata$rem != 'F4', ]
```
...
library(tidyverse)
dbh_height2 <- treedata %>% filter(!is.na(dbh) & !is.na(height))
```
