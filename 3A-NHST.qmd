...
The idea is that we have two normal distributions (e.g. alcohol distributions) to compare means. Let's simulate some data and see the distribution of both groups:
...
text(x = mean(A)-0.4, y = 0.5, labels = "Mean A", pos = 3, xpd = NA, col = "red")
abline(v = mean(B))
text(x = mean(B)+0.4, y = 0.5, labels = "Mean B", pos = 3, xpd = NA, col = "black")
```
...
-   Calculate t-statistic (difference between means / (Standard deviations/sample sizes)):  $t = \frac{\bar{x}_1 - \bar{x}_2}{\sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}}$
...
Usually we also have to test for normality of our data, which we can do with another test, for example the Shapiro-Wilk test:
...
Because we know our data comes from a normal distribution (we simulated from there), the example above is boring. Let's have a look on the normality test with real data:

```{r}
head(PlantGrowth)
boxplot(weight ~ group, data = PlantGrowth)

# let's use only two groups control and treatment1:
...
From the example above, comparing weight of two groups, answer the questions:

1. What is H0? 

2. What is the result?

3. Explain the different values in the output!


::: callout-warning
## Shapiro - Test for normality

If you have a small sample size, the shapiro test will always be non-significant (i.e. not significantly different from a normal distribution)! This is because small sample size leads to low power for rejecting H~0~ of normal distribution. You will see about power in the end of this chapter.
:::

## Type I error rate

Let's start with a small simulation example of comparing two means with t-test. Simulating many times from a world without effect:
...
So in `r mean(results < 0.05)*100` % of our experiments we would reject H~0~ even when there is no effect at all! This is called the type I error rate. Those are **false positives**.
...
-   e.g.: if you test 1000 different genes for their association with cancer, and in reality, none of them is related to cancer, 50 out of the tests will still be significant even if there is no effect.

-   If multiple tests are done, a **correction for multiple testing** should be used.

-   To increase the p-values for each test in a way that the overall alpha level is 0.05:

Let's see it with the `plantGrowth` data:
```{r}
...
We see that two of the three tests are significant at the $\alpha$=0.05 level. But we have to adjust this to avoid false positives. One example is using the `p.adjust` function. This function has 3 methods:
...
# for details on the methods see help of ?p.adjust
```

We end up with only one significant t-test after adjusting for multiple testing, the one comparing treatment 1 and 2. 
...
Now, we wouldn't reject the H~0~ in `r (1-mean(results < 0.05))*100`% of our experiments. This is the **type II error rate (false negatives)**.
...
Before the experiment, you can guess the effect size and the variability, and together with alpha (known), you can calculate the power depending on the sample size:
