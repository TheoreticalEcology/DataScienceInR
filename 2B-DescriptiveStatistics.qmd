...
|------------------------|------------------------|------------------------|
| Numeric | Histogram or boxplot | `hist(num)` and `boxplot(num)` |
| Numeric with numeric | Scatterplot | `plot(num~num)` |
| Numeric with categorical | Boxplot | `boxplot(num~cat)` |
| Categorical with categorical | mosaicplot or grouped barplot | `mosaicplot(table(cat, cat))` or `barplot(data, beside=TRUE)` |

::: column-margin
Here is a [website](https://www.data-to-viz.com/) with a decision tree about when to choose which plot.
:::

Once you know what you want to plot, there are lot of websites that will show you the respective R code. One important consideration, however: there are at least two popular ways of doing R graphs:
...
-   `ggplot2` is a plotting package where the default plots are more visually appealling, it belongs to the `tidyverse`, which is a group of packages with a different grammar for coding in R.
...
::: column-margin
In the base R code we introduced `par()` before plotting to create a panel with the plots side by side. In ggplot2 this is done with the package `ggpubr` (or `patchwork`).
:::
...
plt1 = 
  ggplot(iris, aes(x = Sepal.Length)) +
    geom_histogram(bins = 8,col="white") + # same number of bins from baseR
    ggtitle("Histogram") +
    xlab("Sepal.Length") +
    ylab("Frequency") +
...
::: column-margin
The **formula syntax** can also be used for plot functions in base R.
:::
...
:::column-margin
I "turned off" the legend in plot 2 to be similar to base R plot. Try with `show.legend = T` to see the legend. Also, I had to convert `Month` to a factor to get different shapes.
:::
...
# Hint:
The `notch` argument adds a notch to the boxplot, which gives a visual indication of the confidence interval around the median. If the notches of two boxes do not overlap, it suggests that the medians are significantly different at the 5% significance level.
Try `notch = F` and see what you get. 
:::
