...
Omitted variable bias, what happened with the Temp and Wind effects when in separated models and together in the same model?
```{r}
...
We can see now that the effect of `Temp` on `Ozone` depends on the amount of `Wind`.
